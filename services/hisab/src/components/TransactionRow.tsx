...
function getTransactionIcon(description: string | undefined, isCredit: boolean) {
  ...
  if (/online|amazon|digital|app/i.test(desc)) return IconWorldWww;
  if (/bill|payment|utility/i.test(desc)) return IconReceipt;
  return isCredit ? IconArrowDownLeft : IconArrowUpRight;
}

export default function TransactionRow({ transaction, showCounterparty, runningBalance }: TransactionRowProps) {
  ...
  return (
    ...
    >
      ...
      <Group gap="sm" style={{ flex: 1, minWidth: 0 }} wrap="nowrap">
        <Box
          style={{
            ...
            background: isCredit
              ...
              : 'linear-gradient(135deg, #ffeef0, #ffd4d9)',
            display: 'flex',
            alignItems: 'center',
        ...
        >
          <Icon size={18} color={isCredit ? '#00B894' : '#e17055'} stroke={1.8} />
        </Box>
